...
/// Wrapper for color implementing Serialize and Deserialize
#[derive(Debug, Clone, Copy)]
pub struct Color(pub iced::Color);

impl Color {
    pub fn parse_from_hex(s: impl AsRef<str>) -> Result<Self, &'static str> {
        let s = s.as_ref().trim();
        if !s.is_ascii() {
            return Err("color string must be ascii");
        }
        if !s.starts_with('#') {
            return Err("color string must start with '#'");
        }
        if s.len() != 7 {
            return Err("color string must be 7 characters like '#RRGGBB'");
        }
        let r = u8::from_str_radix(&s[1..3], 16).map_err(|_| "invalid hex for red")?;
        let g = u8::from_str_radix(&s[3..5], 16).map_err(|_| "invalid hex for green")?;
        let b = u8::from_str_radix(&s[5..7], 16).map_err(|_| "invalid hex for blue")?;
        Ok(Self(iced::Color::from_rgb8(r, g, b)))
    }
}
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for ColorVisitor {
            ...
            fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                write!(f, "a color string in the form \"#RRGGBB\"")
            }
            ...
            where
                E: serde::de::Error,
            {
                Color::parse_from_hex(v).map_err(|msg| serde::de::Error::custom(msg))
            }
        }
    ...
    fn default() -> Self {
        Self {
            ...
            colors: HashMap::from(
                ...
                .map(|(name, hex)| (name.to_string(), Color::parse_from_hex(hex).unwrap())),
            ),
        }
    }
}
...
impl Default for Cpu {
    fn default() -> Self {
        Self {
            icon: Icon::new("temperature", ColorNameOrHex::name("lavender")),
            text: None,
        }
    }
}
...
impl Default for Twitch {
    fn default() -> Self {
        Self {
            icon: Icon::new(
                "brand-twitch",
                ColorNameOrHex::Hex(Color::parse_from_hex("#DDB6F2").unwrap()),
            ),
            channels: Vec::new(),
